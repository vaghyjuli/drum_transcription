...
def NMFD(V, W_init, params, L=50, threshold = 0.001):
    """
    ...
    W_init = np.append(W_init, np.ones((K, params["addedCompW"], T)), axis=1)
    R += params["addedCompW"]
    ...
    for iteration in range(L):
        ...
        for t in range(T):
            ...
            if params["fixW"] == "fixed":
                ...
                W[:, :R-params["addedCompW"], t] = (1-alpha) * W_init[:, :R-params["addedCompW"], t] + alpha * W[:, :R-params["addedCompW"], t]
        ...
        W_diff = np.linalg.norm(np.mean(np.abs(W - W_prev), axis=2), ord=2)
        if H_diff < threshold and W_diff < threshold:
            print(f"finished at {iteration}")
            break
